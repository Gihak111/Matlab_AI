...
function [clustered_hn, clustered_reddit, enc] = deep_learning_preprocess_and_cluster(hn_data, reddit_data, enc)
    try
        ...
        clustered_hn = struct('X', features(1:length(hn_data), :), 'Y', labels(1:length(hn_data)));
        clustered_reddit = struct('X', features(length(hn_data)+1:end, :), 'Y', labels(length(hn_data)+1:end));
...
function summarize_results(clustered_hn, clustered_reddit)
    ...
    if isstruct(clustered_hn) && isfield(clustered_hn, 'data') && ~isempty(clustered_hn.data)
        hn_titles = {clustered_hn.data.title};
        hn_links = {clustered_hn.data.link};
    ...
    % Summarize Reddit
    if isstruct(clustered_reddit) && isfield(clustered_reddit, 'data') && ~isempty(clustered_reddit.data)
        reddit_titles = {clustered_reddit.data.title};
        reddit_links = {clustered_reddit.data.link};
