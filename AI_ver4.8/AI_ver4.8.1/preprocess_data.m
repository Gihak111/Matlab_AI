function [X, Y, enc] = preprocess_data(hn_data, reddit_data)
    ...
    % Filter sequences with feature dimension [81 128]
    validIndices = cellfun(@(x) isequal(size(x), [81 128]), sequences);
    sequences = sequences(validIndices);
    labels = labels(validIndices);
    
    % Check if sequences is empty
    if isempty(sequences)
        error('No sequences with feature dimension [81 128] found.');
    end
