function online_learning_unsupervised()
    ...
    for iter = 1:10 % 예시로 10번의 반복
        ...
        if isempty(model)
            ...
            model = create_clustering_model(numClusters);
            
            % 첫 번째 모델 훈련
            model = train_clustering_model(model, clustered_hn, clustered_reddit, options);
        else
            % 기존 모델 업데이트
            model = update_clustering_model(model, clustered_hn, clustered_reddit, options);
        end
...
function [clustered_hn, clustered_reddit, enc] = preprocess_and_cluster(hn_data, reddit_data, enc)
    ...
    if isempty(enc)
        ...
        tokenizer = tokenizedDocument(texts);
        enc = update(enc, tokenizer);
    end
    
    % Convert tokenized documents to bag-of-words representation
    sequences = tokenizedDocument2bag(enc, tokenizer);
    
    % Cluster sequences
    numClusters = 3; % 예시로 3개의 클러스터 사용
    clustering_model = create_clustering_model(numClusters);
    idx = cluster(clustering_model, sequences);
...
function bag = tokenizedDocument2bag(enc, tokenizer)
    numDocs = numel(tokenizer);
    numWords = enc.NumWords;
    bag = zeros(numDocs, numWords);
    
    for i = 1:numDocs
        words = tokenizer(i);
        wordIndices = enc.encode(words);
        
        % Count word frequencies
        freq = histcounts(wordIndices, 1:numWords+1);
        bag(i, :) = freq;
    end
end
...
function model = create_clustering_model(numClusters)
    model = kmeans; % K-means 클러스터링 모델 생성
    % 다른 클러스터링 모델을 사용할 수도 있음
end

function model = train_clustering_model(model, clustered_hn, clustered_reddit, options)
    % 예시로서 실제로 클러스터링 모델을 훈련하지는 않음
    disp('Training clustering model (dummy operation)...');
    model = model; % 실제 모델 훈련 대신 모델을 그대로 반환
end

function model = update_clustering_model(model, clustered_hn, clustered_reddit, options)
