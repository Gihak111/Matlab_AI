classdef TransformerEncoderLayer < nnet.layer.Layer
    properties
        NumHeads
        EmbedDim
        NumTransformers
    ...
    methods
        function obj = TransformerEncoderLayer(numHeads, embedDim, numTransformers, intermediateDim, dropoutRate, name)
        ...
        function Z = predict(obj, X)
            ...
            % Compute softmax
            attentionWeights = softmax(attentionScores, 'DataFormat', 'SSC');

            % Compute attention output
            attentionOutput = pagemtimes(attentionWeights, V);

            % Concatenate heads
            attentionOutput = my_trreshape(attentionOutput, size(attentionOutput, 1), obj.EmbedDim);
